...
/**
 * @brief Modem command handler setup
 *
 * @details Contains user configurable setup which is used to set up
 * command handler. The struct is initialized and then passed to
 ...
 */
struct modem_cmd_handler_setup {
	char *match_buf;
	size_t match_buf_len;
	struct net_buf_pool *buf_pool;
	k_timeout_t alloc_timeout;
